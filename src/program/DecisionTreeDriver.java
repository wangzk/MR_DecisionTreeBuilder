...
public class DecisionTreeDriver {
	...
	public static void main(String[] args) throws Exception {
		//解析命令行里的额外Hadoop相关参数
		if(args.length < 4){
			System.out.println("Usage: DecisionTree.jar MetaFile DataSet TmpWorkingDir TargetModelFilePath");
			return;
		}
		// 存储数据属性元信息的文件的路径
		...
		do {
			...
			String newstatisticFilePath = statisticFilePath + "/static-" + iterateCount;
			...
			runMapReduceJob(dataSetPath, queueFilePath,
					newstatisticFilePath,iterateCount);
			// 从输出结果中读取统计好的信息
			Map<Integer, NodeStatisticInfo> nodeStatisticInfos = new HashMap<Integer, NodeStatisticInfo>();
			...
			for (i = 0; i < Qlength; i++) {
				...
				NodeStatisticInfo info = nodeStatisticInfos.get(new Integer(
						i + 1));
				// 获取一些当前节点的统计信息备用
				...
				if (splitAid == 0) {
					...
					newRule.conditions = new HashMap<Integer, String>(
							rule.conditions);
				...
				for (String value : attributeRange.get(splitAid - 1)) {
					// 判断是否满足终止条件
					String cLabel = satisfyLeafNodeCondition(rule, info,
							splitAid, value);
					// 增加规则
					Rule newRule = new Rule();
					newRule.conditions = new HashMap<Integer, String>(
	...
	private static String satisfyLeafNodeCondition(Rule rule,
		...
		HashSet<Integer> usedAttributes = new HashSet<Integer>(rule.conditions
				.keySet());
		usedAttributes.add(new Integer(splitAid));
		if (usedAttributes.size() == attributeRange.size()) {
			// 新节点具有的多数类
			List<StatisticRecord> newRecords = info.getRecords(new Integer(
					splitAid), value);
		...
		try {
			...
			for (Path path : pathes) {
				...
				while (scanner.hasNextLine()) {
					...
					Integer aid = new Integer(conditionString.split("#")[1]
							.split("\\,")[0]);
					...
					List<StatisticRecord> recordList = nInfo
							.getAttributeStatisticRecords(aid);
					StatisticRecord record = new StatisticRecord(nid, aid,
							avalue, label, count);
	...
	static int findBestSplit(Rule rule, NodeStatisticInfo info) {
		...
		for (Integer aid : info.getAvailableAIDSet()) {
			...
			System.out.println("GainRation[" + aid + "]" + gainA + "/" + splitInfoAD + "=" +  gainRatio);
	...
	/**
	 * 计算records所包含的样本记录集合的信息熵 该函数同时通过DSize参数传回样本集的大小
	 * */
