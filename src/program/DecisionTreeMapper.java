...
public class DecisionTreeMapper extends Mapper<Object, Text, Text, IntWritable> {
	...
	public void setup(Context context) throws IOException {
		...
		System.out.println("FILEPATH:" + filePath + "," + context.getConfiguration());
		System.err.println("Hello:" + context.getConfiguration().get("RuleQueuePath"));
		//载入条件队列信息
	...
	private boolean isFitRule(Rule rule, String[] aValues) {
		...
		for(Integer aid : rule.conditions.keySet()){
			if(!aValues[aid.intValue()-1].equals
					(rule.conditions.get(aid))){
