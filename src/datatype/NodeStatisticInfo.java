...
public class NodeStatisticInfo {
	Map<Integer, List<datatype.StatisticRecord>> infoMap = new HashMap<Integer, List<StatisticRecord>>();
	 ...
	 */
	public List<datatype.StatisticRecord> getAttributeStatisticRecords(
			Integer aid) {
	...
	public static String getMostCommanLabel(List<StatisticRecord> recordList) {
		...
		for (StatisticRecord record : recordList) {
			...
			if (!labelCounts.containsKey(label)) {
				...
				Integer newCount = new Integer(labelCounts.get(label)
						.intValue()
						+ record.count);
		...
		for (String label : labelCounts.keySet()) {
			//System.out.println("CL:" + label + "," + labelCounts.get(label)+",M:" + maxCount);
